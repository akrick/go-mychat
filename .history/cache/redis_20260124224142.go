...
func getUserByUsernameCacheKey(username string) string {
	return fmt.Sprintf("user:by_username:%s", username)
}
...
// 根据用户名获取用户信息（带缓存和singleflight防穿透）
func GetUserByUsernameWithCache(ctx context.Context, username string) (*UserInfoCache, error) {
...
// 删除用户缓存
func DeleteUserCache(ctx context.Context, userID uint, username string) error {
	if err := Rdb.Del(ctx, getUserCacheKey(userID)).Err(); err != nil {
		return err
	}
	if username != "" {
		return Rdb.Del(ctx, getUserByUsernameCacheKey(username)).Err()
	}
	return nil
}
