...
type LowcodeForm struct {
	...
	FormJSON    string `json:"form_json" binding:"required"`
...
func GetFormList(c *gin.Context) {
	...
	query := database.DB.Model(&LowcodeForm{})
...
func DeleteForm(c *gin.Context) {
	...
	if err := database.DB.Delete(&LowcodeForm{}, formID).Error; err != nil {
		c.JSON(500, gin.H{
			"code": 500,
			"msg":  "删除失败",
		})
...
type LowcodePage struct {
	...
	PageJSON  string `json:"page_json" binding:"required"`
	CreatedAt string `json:"created_at"`
	UpdatedAt string `json:"updated_at"`
}
...
func GetPageList(c *gin.Context) {
	...
	query := database.DB.Model(&LowcodePage{})
...
func DeletePage(c *gin.Context) {
	...
	if err := database.DB.Delete(&LowcodePage{}, pageID).Error; err != nil {
		c.JSON(500, gin.H{
			"code": 500,
			"msg":  "删除失败",
		})
