      ...
      >
        ...
        <el-table-column label="用户类型" width="100">
          <template #default="{ row }">
            <el-tag :type="getUserTypeTag(row)">
...
import { getOnlineUsers, broadcastMessage, kickOutUser, sendToUser } from '@/api/system'
...
const handleSendMessage = async () => {
  ...
  sending.value = true
  try {
    await sendToUser(messageForm.userId, { content: messageForm.content })
...
const getUserTypeLabel = (row) => {
  if (row.is_admin) return '管理员'
  if (row.is_counselor) return '咨询师'
  return '普通用户'
}

const getUserTypeTag = (row) => {
  if (row.is_admin) return 'danger'
  if (row.is_counselor) return 'warning'
  return 'primary'
}
